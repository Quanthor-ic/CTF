...
For this challenge, we are given a binary that checks a username against a serial to test if they make a valid pair. We have to reverse the binary to understand how the check is performed in order to write a keygen, and then test it with random usernames by connecting to the given address.
...
Right after that, a few checks are perfomed on the structure of the serial. Firstly the size of the serial must be a mutiple of 3. Then the function at address *0x401a61* is used to check if the serial is composed of some specific characters: the first and second thirds should be only lowercase characters between 'a' and 'p', and the last third should be only constituted of uppercase 'A' or 'B'.
...
After the initialization, each cell of the grids contains only 0, 1 or 2 and from now on, every operation on the grids will be performed modulo 3.
...
The function at *0x4013b1* is used to differentiate the cells of the 2 grids. Whenever a cell has the same value in the parent and in the child, its value is incremented by 1 in the parent grid and by 2 in the child grid.

The next function called is the one at address *0x4015f3*. It perfoms some operations on both grids at the same time based on the value of the serial. The characters of the serial are used 3 by 3, taking one of each third each time. The character from the first third is used to determine the x-axis of a cell by calculating the distance of the character from 'a'. The character from the second third is used in the same way to obtain a y-axis of a cell. Finally the character from the last third is used to select a second cell: if the character is an A, then we choose the cell on the rigth of the previously obtained cell, if it is a B, we choose the one below instead.

Here is an example to make things clearer, with 'c' as the first caracter (x=2), 'b' as the secon (y=1) and A as the third (we use the right neighbour):
...
Some selected cells can be considered as invalid if the serial is incorrect, either because they to are out of bounds or because the check of function *0x4011f2* fails. When the program detects an invalid operation, the return value is incremented by 1.
...
At that point, I knew that some sort of algorithm wad hidden inside these strange series of checks and operations, and that I needed to extract it and define it in a simpler way in order to be able to solve it.
...
That is when I got the key idea to solve the challenge: the grids shouldn't be solved individualy, but instead they should be combineds together by adding them (modulo 3).
...
- The operation performed on the selected cells during function *0x4015f3* changes the color of these 2 cells to the third color.
...
4. If the grid is stuck, then we have to combine a valid cell with an invalid one. Starting from the opposite corner, find the first invalid cell that can be combined with a valid one on the left or on top. After that, go back to step 2.
