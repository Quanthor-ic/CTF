...
I had no previous experience with Dreamcast, so my first goal was trying to understand the content of this file.
...
The file is composed of 2 main parts:
 - IP.BIN, located at the start of the file and of size 0x8000, which acts as a sort of bootloader to start the CD-ROM;
...
3) Screen to input the flag: the input of the user are processed until the A key is pressed (ask for validation)
...
7. The ARM soundcard returns to the SH-4 processor if the check succeeded or not.



### Recovering the secret input: first try

My first idea to obtain the expected input was to implement the cryptographic operations and calculate the result. This seemed like the most obvious way to recover the flag, as I had already understood the complete cryptographic algorithm, however this method turned out to be pretty bad for several reasons.
...
Ater some time, I finally got a good idea: let's try to make a "debugger" by patching an emulator. Flycast has all its source code on github, so we can download it modify, it and hope that it still compiles.

My idea was to try to find a way to make the emulator print the content of the registers and the memory during the check of the input inside the ARM procesor.

I spend quite sometime browsing the source code of Flycast in order to find the correct place to patch. From what I understood, the emulator uses a JIT compiler to compile chunks of ARM code to native instructions in order to improve its performance. The problem with jitted code is that we cannot acces the state of the emulator while it is executing. This means we can see the state of the processor at an address only the first time this code is excuted, because it is not yet jitted.
